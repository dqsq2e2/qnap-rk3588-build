...
static int rockchip_set_ethaddr(void)
{
    ...
    __maybe_unused bool randomed = false;
...
/******************** 条件编译分割线 ********************/
#ifdef CONFIG_ROCKCHIP_SET_ETHADDR
    /* 自定义MAC生成新代码 */
    static const uint8_t MAC_PREFIX[3] = {0x24, 0x5e, 0xbe};
    ...
    if (ret <= 0) {
        ...
        if (!uclass_get_device_by_driver(UCLASS_MISC,
            ...
            misc_read(dev, CFG_CPUID_OFFSET, cpuid, CPUID_LEN);
        }
...
/*         uint8_t low[8], high[8];
        ...
        crc = crc32_no_comp(0, low, 8);
        crc = crc32_no_comp(crc, high, 8); */
        ...
        for (i = 0; i < MAX_ETHERNET; i++) {
            memcpy(&ethaddr[i*ARP_HLEN], MAC_PREFIX, 3);
            ethaddr[i*ARP_HLEN+3] = (crc_low >> 16) & 0xFF;
            ethaddr[i*ARP_HLEN+4] = (crc_low >> 8) & 0xFF;
            ethaddr[i*ARP_HLEN+5] = (crc_low & 0xFF) + i;
            if (i > 0) {
                ethaddr[i*ARP_HLEN] |= 0x02; // 设置本地地址标志
            }
        }
        need_write = true;
    }
...
/******************** 条件编译结束 ********************/
    ...
    /* 原始vendor写入逻辑保留 */
    if (need_write) {
        vendor_storage_write(LAN_MAC_ID, ethaddr, sizeof(ethaddr));
    }
#endif
    return 0;
}
...
static void bootargs_add_fwver(bool verbose)
{
	...
	if (list1 || list2) {
		...
		if (verbose)
			printf("## fwver: %s\n\n", fwver);
		//此处注释掉 env_update,是因为自动加上 androidboot.fwver=ddr-v1.16-9fffbe1e78,bl31-v1.46,uboot-a9c8030796-02/11/2025
