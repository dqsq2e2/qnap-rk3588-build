...
#include <console.h>
#include <mmc.h>
#include <memalign.h>  // 添加缓存对齐头文件
#include <mapmem.h>  // 添加地址映射头文件
...
/* kread命令实现 */
struct kread_header {
	u32 magic;      /* Big-endian 'KREAD' */
	u32 data_size;  /* Big-endian数据长度 */
	u8  reserved[504];
};

static int do_mmc_kread(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	struct blk_desc *blk;
	struct mmc *mmc;
	ulong addr;      // 使用ulong类型代替u32
	u32 data_size;
	ulong sectors_needed;
	int ret;
	ALLOC_CACHE_ALIGN_BUFFER(struct kread_header, hdr, 1);

	if (argc != 3)
		return CMD_RET_USAGE;

	addr = simple_strtoul(argv[1], NULL, 16);
	lbaint_t sector = simple_strtoul(argv[2], NULL, 16);
	...
	blk = mmc_get_blk_desc(mmc);

	/* Step 1: 读取头部分区 */
	ret = blk_dread(blk, sector, 1, hdr);
	if (ret != 1) {
	...
	/* 校验魔数 */
	uint32_t actual_magic = be32_to_cpu(hdr[0].magic);
	...
	data_size = be32_to_cpu(hdr[0].data_size);
	sectors_needed = (data_size + blk->blksz - 1) / blk->blksz;

	/* Step 2: 读取数据区 */
	ret = blk_dread(blk, sector + 1, sectors_needed, map_sysmem(addr, 0));
	return (ret == sectors_needed) ? CMD_RET_SUCCESS : CMD_RET_FAILURE;
}
...
static cmd_tbl_t cmd_mmc[] = {
	...
	U_BOOT_CMD_MKENT(kread, 3, 1, do_mmc_kread, "", ""),
...
U_BOOT_CMD(
	...
	"mmc kread addr sector - Load kernel with header\n"
