 ...
 *
 * QNAP 5.10 backport notes:
 *  - mbus_int/mbus_ext fields dropped: internal PHYs are reached through the
 *    DSA slave mii bus via phy_read/phy_write ops, no self-registered MDIO bus.
 */
...
/* Not present in 5.10 include/net/dsa.h; linked-list tag driver registration
 * makes a plain value sufficient (no enum edit needed).
 */
...
struct yt921x_priv {
	...
	/* cache of cpu ports mask (dsa_cpu_ports() does not exist on 5.10) */
	u16 cpu_ports_mask;
