 ...
 *  - phylink via dsa_switch_ops (phylink_validate/mac_config/mac_link_*),
 *    phylink_get_caps and ds->phylink_mac_ops dropped;
 *    RGMII (XMII) port config added from the full kdev driver version
 *    (CPU port is wired RGMII to the host on BDY G98);
 *  - internal PHYs accessed via phy_read/phy_write ops + DSA slave mii bus,
 *    self-registered internal/external MDIO buses dropped;
 *  - per-port init folded into setup() (no port_setup op in 5.10);
 *  - bridge join/leave use struct net_device *; FDB/MDB ops lose dsa_db;
 *    VLAN add returns void; port_vlan_filtering takes switchdev_trans;
 *    mirror add loses extack; pre_bridge_flags/bridge_flags dropped
 *    (BR_LEARNING is managed by bridge join/leave state instead);
 *  - get_eth_*_stats/get_rmon_stats/get_stats64/get_pause_stats dropped
 *    (no such ops in 5.10), MIB counters still feed get_ethtool_stats;
 *  - ethtool_keee -> ethtool_eee; support_eee dropped;
 *  - container_of_const -> container_of; dsa_cpu_ports()/dsa_port_is_cpu()/
 *    dsa_switch_for_each_user_port()/dsa_port_bridge_dev_get()/
 *    dsa_port_offloads_bridge_dev() replaced by 5.10 equivalents;
 *    ethtool_puts() -> memcpy; dsa_switch_shutdown() dropped;
 *    disable_delayed_work_sync -> cancel_delayed_work_sync.
 */
 ...
 *
 * Unstructured fields (name != NULL) appear in get_ethtool_stats(); we need
 * the sizes and offsets of all fields to perform 32bit MIB overflow
 * wraparound.
 */
...
struct yt921x_info {
	...
	u16 internal_mask;
	/* Ports with SERDES/XMII external interfaces */
	u16 external_mask;
};
...
#define to_yt921x_priv(_ds) container_of(_ds, struct yt921x_priv, ds)
...
static int yt921x_dsa_phy_read(struct dsa_switch *ds, int port, int regnum)
{
	...
	res = yt921x_intif_read(priv, port, regnum, &val);
...
static int
yt921x_dsa_phy_write(struct dsa_switch *ds, int port, int regnum, u16 val)
{
	struct yt921x_priv *priv = to_yt921x_priv(ds);
	int res;
	...
	mutex_lock(&priv->reg_lock);
	res = yt921x_intif_write(priv, port, regnum, val);
	mutex_unlock(&priv->reg_lock);
...
static int yt921x_read_mib(struct yt921x_priv *priv, int port)
{
	...
	for (i = 0; i < ARRAY_SIZE(yt921x_mib_descs); i++) {
		       ...
		       uint8_t *data)
{
	...
	for (i = 0; i < ARRAY_SIZE(yt921x_mib_descs); i++) {
		...
		if (desc->name) {
			memcpy(data, desc->name, ETH_GSTRING_LEN);
			   ...
			   bool ingress)
{
	...
	mutex_lock(&priv->reg_lock);
	res = yt921x_mirror_add(priv, port, ingress, mirror->to_local_port);
	mutex_unlock(&priv->reg_lock);
...
yt921x_dsa_set_ageing_time(struct dsa_switch *ds, unsigned int msecs)
{
	...
	ctrl = clamp_t(unsigned int, msecs / 5000, 1, U16_MAX);
			 ...
			 const struct switchdev_obj_port_vlan *vlan)
{
	...
	/* 5.10 switchdev vlan obj is a range: vid_begin..vid_end */
	mutex_lock(&priv->reg_lock);
	res = 0;
			 ...
			 const struct switchdev_obj_port_vlan *vlan)
{
	...
	/* port_vlan_add is void on 5.10; 5.10 switchdev vlan obj is a range */
	mutex_lock(&priv->reg_lock);
	res = 0;
	for (vid = vlan->vid_begin; vid <= vlan->vid_end; vid++) {
	...
	if (res)
		dev_err(dev, "Failed to %s port %d vid %u: %i\n",
			"add VLAN to", port, vlan->vid_begin, res);
}
...
yt921x_dsa_bridge_ports(struct dsa_switch *ds, const struct net_device *bdev)
{
	...
	for (i = 0; i < ds->num_ports; i++)
		if (dsa_is_user_port(ds, i) &&
		    dsa_to_port(ds, i)->bridge_dev == bdev)
			mask |= BIT(i);
			    ...
			    struct net_device *bridge)
{
	...
	ports_mask = yt921x_dsa_bridge_ports(ds, bridge);
...
static void
yt921x_dsa_phylink_mac_link_down(struct dsa_switch *ds, int port,
...
static void
yt921x_dsa_phylink_mac_link_up(struct dsa_switch *ds, int port,
			       unsigned int mode, phy_interface_t interface,
...
static void
yt921x_dsa_phylink_mac_config(struct dsa_switch *ds, int port,
			      unsigned int mode,
...
static void
yt921x_dsa_phylink_validate(struct dsa_switch *ds, int port,
			    ...
			    struct phylink_link_state *state)
{
	...
	/* Check interface mode against port capabilities */
	if (state->interface != PHY_INTERFACE_MODE_NA) {
		bool ok = false;

		if (info->internal_mask & BIT(port)) {
			/* Port 10 for MCU should probably go here too. But
			 * since that is untested yet, it is turned down.
			 */
			ok = state->interface == PHY_INTERFACE_MODE_INTERNAL;
		} else if (info->external_mask & BIT(port)) {
			/* TODO: external ports may support SERDES only, XMII
			 * only, or SERDES + XMII depending on the chip.
			 * However, we can't get the accurate config table due
			 * to lack of document, thus we simply declare
			 * SERDES + XMII and rely on the correctness of
			 * devicetree for now.
			 */
...
static u32 yt921x_dsa_cpu_ports_mask(struct dsa_switch *ds)
{
	u32 mask = 0;
	int i;
...
static int yt921x_chip_setup(struct yt921x_priv *priv)
{
	...
	/* Enable DSA */
	priv->cpu_ports_mask = yt921x_dsa_cpu_ports_mask(ds);
...
static int yt921x_dsa_setup(struct dsa_switch *ds)
{
	...
	/* 5.10 has no port_setup op; bring up user/cpu ports here after the
	 * chip-wide setup, in the same spirit as the 6.x driver.
	 */
	mutex_lock(&priv->reg_lock);
	...
	for (port = 0; port < ds->num_ports; port++) {
		if (!dsa_is_cpu_port(ds, port) && !dsa_is_user_port(ds, port))
			continue;
...
static const struct dsa_switch_ops yt921x_dsa_switch_ops = {
	...
	/* port phy */
	.phy_read		= yt921x_dsa_phy_read,
	.phy_write		= yt921x_dsa_phy_write,
	/* phylink */
	.phylink_validate	= yt921x_dsa_phylink_validate,
	.phylink_mac_config	= yt921x_dsa_phylink_mac_config,
	.phylink_mac_link_down	= yt921x_dsa_phylink_mac_link_down,
	.phylink_mac_link_up	= yt921x_dsa_phylink_mac_link_up,
	/* port */
...
static void yt921x_mdio_remove(struct mdio_device *mdiodev)
{
	...
	for (i = ARRAY_SIZE(priv->ports); i-- > 0; ) {
